...
mod tests {
    ...
    fn init_legacy_input_signals(
        ...
        for (name, offset, len) in inputs_desc {
            let values = inputs
                .get(name)
                .unwrap_or_else(|| panic!("input signal {} is not found", name));
            assert_eq!(
                values.len(),
                *len,
                "input signal {} has different length",
                name,
            );
            for (i, v) in values.iter().enumerate() {
                signals[*offset + i] = Some(*v);
    ...
    fn execute_legacy_fixture(vm_data: &[u8], inputs_data: &[u8]) -> Vec<u8> {
        let inputs = crate::deserialize_inputs(inputs_data).unwrap();
        let circuit = deserialize_witnesscalc_vm(vm_data).unwrap();
        ...
        init_legacy_input_signals(&circuit.inputs, &inputs, &mut signals);
        ...
        let witness = circuit
            .witness_signals
            .iter()
            .map(|idx| signals[*idx].unwrap())
            .collect();
        crate::wtns_from_u256_witness(witness)
    }

    #[test]
    fn execute_vm_matches_real_multi_template_fixture_witness() {
        let vm_data = include_bytes!("../tests/vm1/data/circuit3_vm.wcd");
        let inputs_data = include_bytes!("../test_circuits/circuit3_inputs.json");
        // Generated by Circom's WASM witness generator for circuit3.
        let expected = include_bytes!("../tests/vm1/data/circuit3_expected.wtns");

        let got = execute_legacy_fixture(vm_data, inputs_data);
